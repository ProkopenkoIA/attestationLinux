...
import static animals.view.View.AddCommandMenu.ADD_SKILL;
...
public class ConsoleView implements View {
    ...
    private final FarmAccounting farmAccounting;
    private Scanner scanner;

    public ConsoleView(FarmAccounting farmAccounting) {
        this.farmAccounting = farmAccounting;
        scanner = new Scanner(System.in);
    }

    record AnimalData(String name, LocalDate birthDate, AnimalType animalType){}

    @Override
    public void showKennelRegistry() {
        clearConsole();
        printLineWithSymbol("=", SIZE_LINE);
        printCaption("Животные", " ");
        printLineWithSymbol("=", SIZE_LINE);
        printRegistryHeader();
        printLineWithSymbol("-", SIZE_LINE);

        List<AbstractAnimal> animals = farmAccounting.getAnimals();
    ...
    public boolean showAddAnimalDialog() {
        String infoMessage = "Введите параметры животного в виде строки: \"имя день_рождения род_животного\"\n" +
                "день_рождения имеет вид: dd-mm-yyyy (01-01-2000): \n" +
        ...
        while (true) {
            ...
            try (counter){
                ...
                counter.add();
                return farmAccounting.createAnimal(parsedData.name, parsedData.birthDate, parsedData.animalType);
    ...
    public void showDetailInfoAnimalDialog() {
        String infoMessage = String.format("Введите номер животного (1 - %d)", farmAccounting.getAnimals().size());
        System.out.println(infoMessage);
        while (true) {
            try {
                scanner = new Scanner(System.in);
                int animalNumber = scanner.nextInt();
                AbstractAnimal animal = farmAccounting.getAnimals().get(animalNumber - 1);
    ...
    public void showAnimalInfo(AbstractAnimal animal) {
        while (true) {
            ...
            AddCommandMenu code = showAddComandMenu(animal);
            switch (code) {
                case ADD_SKILL -> {
    ...
    @Override
    public AddCommandMenu showAddComandMenu(AbstractAnimal animal) {
        String menu = String.format(
                "%d-[%s]\t%d-[%s]\n",
                AddCommandMenu.ADD_SKILL.ordinal(), AddCommandMenu.ADD_SKILL.getTag(),
                AddCommandMenu.EXIT.ordinal(), AddCommandMenu.EXIT.getTag()) ;
        printLineWithSymbol("=", SIZE_LINE);
        printColorLine("Доступные действия:", GREEN_COLOR);
        printColorLine(menu, GREEN_COLOR);
        while (true) {
            try {
                System.out.printf("%s (%d - %d): ", "Выберите действие: ", AddCommandMenu.ADD_SKILL.ordinal(),
                        AddCommandMenu.EXIT.ordinal());
                scanner = new Scanner(System.in);
                return AddCommandMenu.values()[scanner.nextInt()];
            } catch (ArrayIndexOutOfBoundsException | InputMismatchException e) {
                printColorLine("Некоректное действие!", RED_COLOR);
            }
        }
    ...
    @Override
    public boolean showAddComandDialog(AbstractAnimal animal) {
        ...
        if (commandData.length == 1) {
            animal.learnCommand(new Command(commandData[0]));
        } else if (commandData.length == 2) {
            animal.learnCommand(new Command(commandData[0], commandData[1]));
        } else {
    ...
    public int showRemoveAnimalDialog() {
        ...
        while (true) {
            try {
                ...
                return farmAccounting.removeAnimal(animal);
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Запись с таким номером отсутствует");
            }
        }
     ...
     *                  род_животного - содержит значения из пепечисления AnimalType
    ...
    private AnimalData parceAnimalData(String inputData) {
        ...
        AnimalType animalType = AnimalType.valueOf(input[2].toUpperCase());
        return new AnimalData(animalName, birthDay, animalType);
    }
